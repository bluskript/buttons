...
export function rotate(points: ReadonlyVec3[], rx: number, ry: number, rz: number) {
	...
	const rzmat = mat3.fromValues(
		1, 0, 0,
		0, Math.cos(rz), -Math.sin(rz),
		0, Math.sin(rz), Math.cos(rz)
	)

	return points.map(p => {
		let final = vec3.transformMat3(vec3.create(), p, rxmat)
		final = vec3.transformMat3(final, p, rymat)
		final = vec3.transformMat3(final, p, rzmat)
		return final
	})
}
